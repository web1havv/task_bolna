...
class AddressWeightedTrainer(Seq2SeqTrainer):
    def __init__(self, alpha_start: float = 1.0, alpha_end: float = 2.5, **kwargs):
    ...
    def _update_alpha(self) -> None:
        # Gradually increase address weighting up to alpha_end.
        # Uses global_step / max_steps as progress proxy.
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if address_mask is not None:
            ...
            # Cap at <= 2.5x overall weighting (alpha_end default is 2.5)
...
def create_lora_config() -> LoraConfig:
    # Decoder LoRA: keep rank low, focus only on attention q/v to reduce hallucination risk.
    return LoraConfig(
        r=16,
        lora_alpha=32,
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Curriculum stage 1 epochs (train on text_phoneme).",
    )
    parser.add_argument(
        "--canonical-epochs",
        type=int,
        default=8,
        help="Curriculum stage 2 epochs (train on text_canonical).",
    )
    parser.add_argument(
        "--max-train-samples",
        type=int,
    ...
    print("[bold]Starting curriculum training...[/bold]")
    # Phase 1: phoneme-faithful
