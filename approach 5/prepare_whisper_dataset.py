...
def sanitize_record(rec: Dict) -> Optional[Dict]:
    ...
    return {
        ...
        # Stage-1 label (phoneme-faithful, matches what TTS said)
        "text": text,
        # Curriculum labels (if present)
        "text_phoneme": (rec.get("text_phoneme") or text).strip(),
        "text_canonical": (rec.get("text_canonical") or text).strip(),
        ...
        "address_position": (rec.get("address_position") or "").strip(),
        "variant_type": (rec.get("variant_type") or "").strip(),
...
def main(argv: Optional[List[str]] = None) -> None:
    parser = argparse.ArgumentParser(
        description="Prepare JSONL dataset for Whisper fine-tuning (Approach 4: train/test/eval)."
    )
    parser.add_argument(
        "--max-train",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional max number of test examples.",
    )
    parser.add_argument(
        "--max-eval",
        type=int,
    ...
    test_out = WHISPER_DATASET_DIR / "test.jsonl"
    ...
    print(
        ...
        f"{len(train_records)} train, {len(test_records)} test, {len(eval_records)} eval examples.[/green]"
    )
