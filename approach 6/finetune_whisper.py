"""
Approach 6: Improved Fine-tuning for Bangalore Address Transcription

Key improvements over Approach 5:
1. Skip phoneme curriculum (was confusing the model)
2. Focal loss on address tokens (focus on hard examples)
3. Sequence-level address bonus (reward full address matches)
4. Label smoothing (reduce overconfidence)
5. Wider LoRA targets (add k_proj for more capacity)
6. R-Drop consistency loss (regularization for generalization)
7. Gradient checkpointing option (memory efficiency)

Goal: Improve canonical address accuracy while maintaining token F1 gains.
"""
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=16,
        help="LoRA rank for decoder (16-24 recommended).",
    )
    parser.add_argument(
        "--no-k-proj",
        action="store_true",
