# Whisper Fine-tuning for Indian Address Recognition

Fine-tuning OpenAI's Whisper model for improved recognition of Indian addresses (Bangalore) using LoRA (Low-Rank Adaptation).

**Hardware Used:** Google Colab with T4 GPU

---

## Best Results (Approach 7)

| Metric | Value | Change |
|--------|-------|--------|
| **WER** | — | +0.0438 (improvement) |
| **Canonical Accuracy** | — | +4.40% (improvement) |
| **Token F1** | — | +0.0274 (improvement) |
| **Span F1** | 0.4870 | Best across all approaches |
| **Phoneme-aware WER** | 0.0907 | improvement |

**Approach 7 is the only setup where accuracy actually goes up, and all other metrics also improve.**
...
# Set Sarvam API key (for audio generation)
export SARVAM_API_KEY="your-key-from-dashboard.sarvam.ai"

# Generate addresses
python generate_addresses.py --skip-india-post

# Generate sentences
python generate_sentences.py --total 2500

# Generate audio (requires SARVAM_API_KEY)
...
- Sarvam AI TTS API (for audio generation)

See `requirements.txt` in each approach folder for full dependencies.
