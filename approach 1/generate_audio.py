...
def synthesize_gtts(
    ...
    More defensive:
    - retries on transient gTTS/ffmpeg issues
...
def normalize_simple(audio: AudioSegment) -> AudioSegment:
    """
    ...
    - no added noise
    - no random silences
    Just make sure level is reasonable and 16kHz mono.
    """
...
def build_call_audio(transcript: str) -> AudioSegment:
    """
    TTS the full conversation transcript as a single utterance.
    You can later extend this to per-turn TTS with pauses.
    """
...
def main(argv: Optional[List[str]] = None) -> None:
    parser = argparse.ArgumentParser(
        description="Generate synthetic telephone-style audio for conversations using gTTS."
    )
    parser.add_argument(
        "--max-train",
        type=int,
