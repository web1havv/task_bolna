...
def spoken_variants_for(canonical: str) -> List[str]:
    ...
    Uses phonetic-level spelling tricks to help gTTS pronounce names correctly.
    Includes:
    - Number variations (4th -> fourth / four)
    - Acronym expansion (HSR -> h s r)
    - Phonetic spellings for common Bangalore localities
    - Syllable breaks to guide pronunciation
    ...
    # Acronym expansions (phonetic letter-by-letter)
    acronym_map = {
        "hsr": ["h s r", "h s r layout", "h s r layout"],
        "btm": ["b t m", "b t m layout", "b t m layout"],
        "rmv": ["r m v", "r m v layout"],
        "jpn": ["j p n", "j p nagar"],
        "jpnagar": ["j p nagar", "j p n nagar"],
        "jpnagar": ["j p nagar", "j p n nagar"],
    }
    for key, repls in acronym_map.items():
        if key in text:
            for r in repls:
                variants.add(text.replace(key, r))

    # Comprehensive phonetic spellings for Bangalore localities
    # These help gTTS understand correct pronunciation
    phonetic_map = {
        # Common localities with phonetic variants
        "koramangala": [
            "koraman gala",           # Syllable break
            "koram angala",           # Alternative break
            "kora mangala",           # Different break
            "koramangala",            # Original
        ],
        "indiranagar": [
            "indira nagar",           # Syllable break
            "indir nagar",            # Shortened
            "indira nagar",           # Standard
        ],
        "jayanagar": [
            "jaya nagar",             # Syllable break
            "jayan nagar",            # Alternative
        ],
        "rajajinagar": [
            "raja jinagar",           # Syllable break
            "raja ji nagar",          # Triple break
            "rajaji nagar",           # Alternative
        ],
        "basavanagudi": [
            "basava nagudi",          # Syllable break
            "basavan agudi",          # Alternative break
        ],
        "malleshwaram": [
            "mallesh waram",          # Syllable break
            "malleshwara m",          # Alternative
        ],
        "sadashivanagar": [
            "sadashiva nagar",        # Syllable break
            "sadashiv nagar",         # Shortened
        ],
        "vijayanagar": [
            "vijaya nagar",           # Syllable break
            "vijayan nagar",          # Alternative
        ],
        "hebbal": [
            "hebbal",                 # Original
            "hebb al",                # Syllable break
        ],
        "whitefield": [
            "white field",             # Word break
            "whitefield",             # Original
        ],
        "marathahalli": [
            "maratha halli",          # Syllable break
            "marathah alli",          # Alternative break
        ],
        "electronic city": [
            "electronic city",         # Original
            "electronics city",       # Plural variant
        ],
        "bengaluru": [
            "bangalore",              # English name
            "bengaluru",              # Kannada name
        ],
        "bengaluru urban": [
            "bangalore urban",        # English variant
        ],
        # Common road/area names
        "brigade": [
            "brigade",                # Original
            "brigade road",           # With road
        ],
        "mg road": [
            "m g road",               # Letter-by-letter
            "mg road",                # Original
            "mahatma gandhi road",    # Full form
        ],
        "hosur road": [
            "hosur road",             # Original
            "hosur highway",          # Alternative
        ],
        # Stage/block variations
        "stage": [
            "stage",                  # Original
            "st stage",               # Abbreviated
        ],
        "block": [
            "block",                  # Original
            "blk",                    # Abbreviated
    ...
    # Apply phonetic mappings (case-insensitive matching)
    for key, repls in phonetic_map.items():
        ...
        if f" {key} " in text:
            for r in repls:
                variants.add(text.replace(f" {key} ", f" {r} "))
        # Try at start
        if text.startswith(key + " "):
            for r in repls:
                variants.add(text.replace(key + " ", r + " ", 1))
        # Try at end
        if text.endswith(" " + key):
            for r in repls:
                variants.add(text[::-1].replace((key + " ")[::-1], (r + " ")[::-1], 1)[::-1])

    # Add syllable-break variants for compound words (heuristic)
    # Split on common patterns that help pronunciation
    words = text.split()
    for i, word in enumerate(words):
        if len(word) > 6:  # Long words benefit from breaks
            # Try breaking at common syllable boundaries
            if "nagar" in word:
                broken = word.replace("nagar", " nagar")
                if broken != word:
                    new_words = words.copy()
                    new_words[i] = broken
                    variants.add(" ".join(new_words))
            if "gala" in word:
                broken = word.replace("gala", " gala")
                if broken != word:
                    new_words = words.copy()
                    new_words[i] = broken
                    variants.add(" ".join(new_words))
            if "halli" in word:
                broken = word.replace("halli", " halli")
                if broken != word:
                    new_words = words.copy()
                    new_words[i] = broken
                    variants.add(" ".join(new_words))
    ...
    for v in variants:
        cleaned.add(
            " ".join("".join(ch for ch in word if ch.isalnum() or ch.isspace()).strip() 
                    for word in v.split()).strip()
        )

    # Ensure min 5 and max 15 variants
