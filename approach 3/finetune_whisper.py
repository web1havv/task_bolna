...
def create_lora_config() -> LoraConfig:
    ...
    # PEFT uses substring matching on full module names
    # For Whisper: model.decoder.layers.{N}.self_attn.{q,k,v,o}_proj
    # Since encoder is frozen, we can use just the projection names
    # PEFT will only apply to Linear layers matching these names in decoder
    return LoraConfig(
        r=32,
        lora_alpha=64,
        target_modules=[
            ...
            "v_proj",
            "o_proj",
        ],  # These will match decoder.layers.*.self_attn.{q,k,v,o}_proj (encoder is frozen)
        lora_dropout=0.05,
        bias="none",
...
def main():
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        include_inputs_for_metrics=False,  # Don't add extra inputs
